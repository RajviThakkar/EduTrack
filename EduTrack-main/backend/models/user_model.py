...
def get_users_by_roles(roles):
    if _is_mysql_backend():
        ...
        try:
            ...
            query = f"SELECT user_id, name, email, role, created_at FROM users WHERE role IN ({placeholders})"
            cursor.execute(query, tuple(roles))
            return [_serialize_user(doc) for doc in cursor.fetchall()]
        finally:
            conn.close()

    docs = _users_collection().find({"role": {"$in": roles}})
